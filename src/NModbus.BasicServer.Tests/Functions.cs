...
public class Functions
{
    ...
    public async Task WriteHoldingRegistersShouldWork(ushort startingAddress, ushort[] values)
    {
        ...
        _ = await implementation.ProcessAsync(request, default);

        storageMock.Verify(m => m.WritePoints(startingAddress, values), Times.Once);
    }

    [Theory]
    [InlineData(50, new[] { true, true, false, false, false, false, false, true })]
    public async Task ReadCoilsShouldWork(ushort startingAddress, bool[] values)
    {
    ...
    [Theory]
    [InlineData(50, true)]
    public async Task WriteSingleCoilShouldWork(ushort outputAddress, bool outputValue)
    {
    ...
    [Theory]
    [InlineData(100, new[] { true, false, false, false })]
    public async Task WriteMultipleCoilsShouldWork(ushort startingAddress, bool[] values)
    {
        var storageMock = new Mock<IDevicePointStorage<bool>>();
        ...
        _ = await implementation.ProcessAsync(request);
    ...
    public async Task ReadWriteMultipleRegistersNonOverlappingShouldWork(ushort readStartingAddress,
        ushort[] readValues, ushort writeStartingAddress, ushort[] writeValues)
    {
        var storageMock = new Mock<IDevicePointStorage<ushort>>();

        _ = storageMock.Setup(
                s => s.ReadPoints(readStartingAddress, (ushort)readValues.Length)
            )
            .Returns(readValues);

        var implementation = new ReadWriteMultipleRegistersImplementation(_loggerFactory, storageMock.Object);

        var request = new ReadWriteMultipleRegistersRequest(readStartingAddress, (ushort)readValues.Length,
            writeStartingAddress, writeValues);
    ...
    [Theory]
    [InlineData(1000, new[] { false, true, true, false })]
    public async Task ReadDiscreteShouldWork(ushort startingAddress, bool[] expectedValues)
    {
        var storageMock = new Mock<IDevicePointStorage<bool>>();

        _ = storageMock.Setup(
            s => s.ReadPoints(startingAddress, (ushort)expectedValues.Length)
